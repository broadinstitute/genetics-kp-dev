...
def insert_pathway(conn, pathway_code, pathway):
    ...
    '''
    sql_insert = """
    insert into {}.{} (pathway_code, pathway_name, exact_source, pmid, systematic_name, msig_url, gene_count, pathway_prefix, ontology_id)
    ...
    pathway_prefix = pathway_code.split("_")[0]
    ontology_id = get_ontology_id(pathway_prefix, pathway)
...
def get_ontology_id(prefix, pathway):
    '''
    will return a pathway ontology id if there is one, None otherwise
    '''
    # initialize
    ontology_id = None

    # get the ontology_id
    if prefix in ['GOBP', 'GOCC', 'GOMF', 'HP']:
        ontology_id = pathway.get('exactSource')
    if prefix in ['REACTOME']:
        ontology_id = 'REACTOME:' + pathway.get('exactSource')
    if prefix in ['KEGG']:
        ontology_id = 'KEGG:' + pathway.get('exactSource')
    if prefix in ['PID']:
        ontology_id = 'PID:' + pathway.get('pmid')
    if prefix in ['BIOCARTA']:
        ontology_id = 'BIOCARTA:' + pathway.get('systematicName')
    if prefix in ['WP']:
        ontology_id = pathway.get('exactSource')[0:2] + ':' + pathway.get('exactSource')[2:]
...
# functions
def create_updated_name(name, log=False):
    '''
    ...
    new_name = new_name.replace("Kegg ", "Kegg - ")
    new_name = new_name.replace("Biocarta ", "Biocarta - ")
    new_name = new_name.replace("Wp ", "WikiPathways - ")
    new_name = new_name.replace("Pid ", "Pid - ")
    new_name = new_name.replace("Hp ", "HP - ")
