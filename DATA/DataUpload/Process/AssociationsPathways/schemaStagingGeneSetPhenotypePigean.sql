...
drop table if exists tran_upkeep.agg_gene_set_phenotype_pigean;
create table tran_upkeep.agg_gene_set_phenotype_pigean (
  id                           int not null auto_increment primary key,
  gene_set_code                varchar(250) not null,
  phenotype_code               varchar(250) not null,
  beta_uncorrected             double,
  beta                         double,
  n                            integer(9),
  date_created                 datetime DEFAULT CURRENT_TIMESTAMP
);

alter table tran_upkeep.agg_gene_set_phenotype_pigean add index gene_phen_gene_set_pig_cde_idx (gene_set_code);
alter table tran_upkeep.agg_gene_set_phenotype_pigean add index gene_phen_phen_set_pig_cde_idx (phenotype_code);
