...
# constants
# url_query_aggregator_by_gene = "https://bioindex-dev.hugeamp.org/api/bio/query/pigean-gene?q={},2,small"
url_query_aggregator_by_phenotype = "https://bioindex-dev.hugeamp.org/api/bio/query/pigean-gene-phenotype?q={},2,small"
...
def get_phenotype_list(conn):
    '''
    retrieves the portal based phentype list
    '''
    list_result = []

    # query
    sql_select = "select phenotype_id, phenotype_name from {}.agg_aggregator_phenotype".format(DB_SCHEMA)
    cursor = conn.cursor()
    ...
    # build the list
    if db_results:
        list_result = [{'id': item[0], 'phenotype': item[1]} for item in db_results]
...
def insert_gene_associations(conn, list_gene_assoc, log=False):
    ...
    '''
    sql_insert = """
        insert into {}.agg_gene_phenotype_pigean (gene_code, phenotype_code, score_huge, combined, log_bf, prior, n) values(%s, %s, %s, %s, %s, %s, %s) 
        """.format(DB_SCHEMA)
    ...
    for gene_association in list_gene_assoc:
        ...
        log_bf = gene_association.get('log_bf')
        prior = gene_association.get('prior')
        n = gene_association.get('n')
        ...
        cur.execute(sql_insert, (gene, phenotype, hugeScore, probability, log_bf, prior, n))
...
def rest_get_phenotype_associations(phenotype, log=True):
    '''
    will query the bioindex rest service for te gene/phenotype association data
    '''
    # initialize
    ...
    # query the service
    response = requests.get(url_query_aggregator_by_phenotype.format(phenotype_id)).json()
    list_data = response.get('data')

    # loop and build list
    for row in list_data:
        list_result.append({'gene': row.get('gene'), 'phenotype': row.get('phenotype'), 'score_huge': row.get('huge_score'), 
                            'combined': row.get('combined'), 'log_bf': row.get('log_bf'), 'prior': row.get('prior'), 'n': row.get('n')})

    # log
    if log:
        print("for phenotype: {} got results of count: {}".format(phenotype_id, len(list_data)))
...
if __name__ == "__main__":
    ...
    # log
    print("got phenotype list of size {}".format(len(list_phenotype)))
    ...
    for phenotype in list_phenotype:
        if num_count < 0 or count_phenotype < num_count:
            count_phenotype = count_phenotype + 1

            # get the gene data
            list_data = rest_get_phenotype_associations(phenotype=phenotype)

            # insert the data in the upkeep db
            insert_gene_associations(conn=conn, list_gene_assoc=list_data)

            print("for {}/{} - {} got new gene/phenotype associations of size {}\n".format(count_phenotype, num_phenotypes, phenotype.get('phenotype'), len(list_data)))
