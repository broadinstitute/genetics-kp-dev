...
# constants
url_query_aggregator = "https://bioindex-dev.hugeamp.org/api/bio/query/gene-associations?q={}"
...
def get_gene_list(conn, is_incremental=False, log=False):
    ...
    if is_incremental:
        sql_string = """
        select gene.node_code, gene.ontology_id from {}.agg_gene gene where not exists (select id from {}.agg_gene_phenotype_magma magma where magma.gene_code = gene.node_code collate utf8mb4_unicode_ci)
...
def delete_gene_associations_magma(conn):
    ''' 
    delete the gene/phenotype associations from the agregator load table
    '''
    sql_delete = """
    delete from {}.agg_gene_phenotype_magma 
    """.format(DB_SCHEMA)
...
def insert_gene_associations(conn, list_gene_assoc, log=False):
    ...
    '''
    sql_insert = """
        insert into {}.agg_gene_phenotype_magma (gene_code, phenotype_code, gene_type, z_stat, p_value) values(%s, %s, %s, %s, %s) 
        """.format(DB_SCHEMA)
    ...
    for gene_association in list_gene_assoc:
        ...
        pValue = gene_association.get('p_value')
...
def log_gene_associations_data_counts(conn):
    ...
    # log the number
    cursor = conn.cursor()
    sql_to_run = sql_count.format(DB_SCHEMA, "agg_gene_phenotype_magma")
...
def rest_get_gene_associations(map_gene, log=True):
    ...
    for row in list_data:
        if row.get('ancestry') and row.get('ancestry') == 'Mixed':
            list_result.append({'gene': row.get('gene'), 'phenotype': row.get('phenotype'), 'p_value': row.get('pValue'), 
                                'zstat': row.get('zStat'), 'type': row.get('type')})

    # log
    if log:
        print("for gene: {} got results of count: {} with mixed count: {}".format(gene, len(list_data), len(list_result)))
...
if __name__ == "__main__":
    ...
    if not is_incremental:
        print("deleting table tran_upkeep.agg_gene_phenotype_magma")
        delete_gene_associations_magma(conn)
